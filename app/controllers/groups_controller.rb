class GroupsController < ApplicationController
  ...
  def daily_total_expenses
    current_user.entities.where('created_at >= ? AND created_at < ?', Time.now.beginning_of_day,
                                Time.now.end_of_day).sum(:amount)
  end

  helper_method :daily_total_expenses
end
